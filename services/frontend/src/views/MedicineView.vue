<template>
  ...
  <section>
    ...
    <!-- <p> -->
        <!-- &nbsp;&nbsp; -->
        <!-- <router-link :to="{name: 'AddPharmacy'}" class="btn btn-primary">添加</router-link> -->
    <!-- </p> -->
    <hr/><br/>
    <div v-if="onsales && onsales.length"></div>
...
export default defineComponent({
  ...
  methods: {
    ...mapActions(['deleteOnsale', 'getOnsalesByPharmacy']),
    async removeOnsale(OnsaleId) {
      try {
        await this.deleteOnsale(OnsaleId);
        await this.$store.dispatch('getOnsalesByPharmacy', this.pharmacyId); //刷新mypharmacies
      } catch (error) {
        console.error(error);
      }
    }
